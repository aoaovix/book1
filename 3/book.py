...
def retrieve_aozora(url):
    print(f'Retrieve Aozora:{url}.')
    return get_aozora(url)


@st.cache_data
def prepare_pos_ids():
    print(f'Retrieve POS_IDs')
    return get_pos_ids()
...
if aozora_url is not None:
    ...
    with tab_sentiment:
        ...
        sentences = {key:0 for key in SENTIMENTS}

        for idx, sentences in enumerate(sentences):
            emotion = get_sentiment(pipe,sentences)
            bar.progress(value=idx/len(sentences),text='計算中')
            sentiments[emotion] = sentiments[emotion] + 1

            if trace_on is True:
                st.markdown(f'{sentences}==>{emotion}')
